...
public class LaihuolaController {
    ...
    public Mono<BaseR<LoginResp>>
    login(@RequestParam("phone") String phone, @RequestParam("captcha") String captcha) {
        return service.login(phone, captcha);
    }

    @GetMapping(path = "/captcha")
    public Mono<BaseR>
    captcha(@RequestParam("phone") String phone) {
        return service.captcha(phone);
    }

    @GetMapping(path = "/goods/all")
    public Mono<BaseR<DataR<List<GoodsResp>>>>
    goods(@RequestHeader(value = "Token") String token,
